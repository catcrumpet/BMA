"bic.surv" <- function(x, ...) UseMethod("bic.surv")
    ...
    function(x, surv.t, cens, strict = FALSE, OR = 20, maxCol = 30,
        ...
        leaps.bs <- function(info, coef, names.arg, nbest = nbest) {
            ...
            ans <- .Fortran("fwleaps", as.integer(nv), as.integer(it), 
                            ...
                            as.integer(1), as.double(2), as.integer(mb), as.double(rt), 
                            as.integer(nd), as.integer(nc), as.integer(iw), 
                            as.integer(nw), as.double(rw), as.integer(nr), as.double(t1), 
                            as.double(s2), as.integer(ne), as.integer(iv), as.double(Subss), 
                            as.double(RSS), as.integer(nret), PACKAGE = "BMA")
            regid <- ans[[21]] / 2
            r2 <- ans[[20]]
            nreg <- sum(regid > 0)
            regid <- regid[1:nreg]
            ...
            z <- regid
            which <- matrix(as.logical((rep.int(z, kx) %/% rep.int(2^((kx - 1):0), rep.int(length(z), kx))) %% 2), 
                            byrow = FALSE, 
                            ncol = kx)
            size <- which %*% rep(1, kx)
            label <- character(nreg)
            sep <- 
                if (all(nchar(names.arg) == 1)) {
                    ""
                } else {
                    ","
                }
            for (i in 1:nreg) {
                label[i] <- paste(names.arg[which[i, ]], collapse = sep)
            }
        ...
        dropcols <- function(x, surv.t, cens, maxCols = 30) {
            ...
            for (i in 2:n.designx) {
                if (isfac[designx[i]]) {
                    designx.levels[i] <- sum(designx[1:i] == designx[i]) + 1
                }
            }
            
            x.coxph <- data.frame(mm[, -1], surv.t = surv.t, cens = cens)
            cox.out <- coxph(Surv(surv.t, cens) ~ ., 
                             data = x.coxph, 
                             method = "breslow", 
                             iter.max = 30)
            ...
            xx <- data.frame(matrix(rep(NA, times = new.nvar * nrow(x.coxph)), ncol = new.nvar))
            ...
            for (i in 1:new.nvar) {
                ...
                if (isfac[cvar]) {
                    if (length(lvls) != length(levels(x[, cvar]))) {
                        newvar <- (as.matrix(x.coxph[, cvar == designx[-1]]) %*% cbind(lvls - 1)) + 1
                        ...
                        dropped.levels <- c(dropped.levels, 
                                            paste(vnames[cvar], "_", removed.levels, sep = ""))
                    } else {
                        xx[, i] <- factor(x[, cvar])
                        new.names[i] <- vnames[cvar]
                    }
        ...
        if (factors) {
            ...
            cox.out <- coxph(Surv(surv.t, cens) ~ ., data = x.coxph, method = "breslow")
            ...
            if (factor.type) {
                for (i in 1:length(names(x))) {
                    if (!is.null(fn[[i]])) {
                        ...
                        old.vals <- x[, i]
                        new.vals <- c(0, coefs)
                        new.vec <- as.vector(new.vals[match(old.vals, fn[[i]])])
                        leaps.x[, nx] <- new.vec
                    }
                }
            } else {
                ...
                for (i in 1:length(names(x))) {
                    ...
                    if (!is.null(fn[[i]])) {
                        ...
                        if (factor.prior.adjust) {
                            addprior <- rep(1 - (1 - prior.param[i])^(1/(k - 1)), k - 1)
                        } else {
                            addprior <- rep(prior.param[i], k - 1)
                        }
                    }
        ...
        cox.assign <- coxph(Surv(surv.t, cens) ~ ., 
                            data = x.coxph.fac, 
        ...
        if (ncol(leaps.x) >= 3) {
            ...
            nmod <- length(a$size)
            prior.mat <- matrix(rep(prior.param, nmod), nmod, ncol(x), byrow = TRUE)
            ...
            for (k in 1:nmod) {
                if (k == 1) {
                    ...
                    label[k] <- paste(names(leaps.x)[which[k, ]], collapse = sep)
                }
            }
        }
        ...
        for (k in (1:length(label))) {
            if (sum(which[k, ]) != 0) {
                ...
                model.fits[[k]] <- matrix(rep(0, 2 * length(cox.out$coef)), ncol = 2)
        ...
        for (i in (1:ncol(x))) {
            ...
            if (sum(sel) > 0) {
                ...
                CEbi[cox.assign[[i + 1]]] <- as.numeric(cpp %*% EbiMk[sel, cox.assign[[i + 1]]])
                CSDbi[cox.assign[[i + 1]]] <- sqrt(cpp %*% (sebiMk[sel, cox.assign[[i + 1]]]^2) + 
                                                       cpp %*% ((EbiMk[sel, cox.assign[[i + 1]]] - 
                                                                     CEbi[cox.assign[[i + 1]]])^2))
            }
        }
...
"bic.surv.formula" <-
    function(f, data, strict = FALSE, OR = 20, maxCol = 30, 
             prior.param = c(rep(0.5, ncol(x))), OR.fix = 2, nbest = 150, 
             factor.type = TRUE, factor.prior.adjust = FALSE, call = NULL, ...) {
        cl <- match.call()
        ...
        moddata <- moddata[, -1]
        cnames <- gsub(":", ".", cnames)
        moddata <- cbind(moddata, datalist[[1]][, 1], datalist[[1]][, 2])
...
"bic.surv.matrix" <-
    function(x, surv.t, cens, strict = FALSE, OR = 20, maxCol = 30, 
        ...
        leaps.bs <- function(info, coef, names.arg, nbest = nbest) {
            ...
            which <- matrix(TRUE, nreg, kx)
            z <- regid
            which <- matrix(as.logical((rep.int(z, kx) %/% rep.int(2^((kx - 1):0), rep.int(length(z), kx))) %% 2), byrow = FALSE, ncol = kx)
            size <- which %*% rep(1, kx)
            label <- character(nreg)
        ...
        if (ncol(leaps.x) >= 3) {
            ...
            prior.mat <- matrix(rep(prior.param, nmod), nmod, ncol(x), 
                                byrow = TRUE)
            prior <- apply(a$which * prior.mat + (!a$which) * (1 - prior.mat), 
                           1, 
                           prod)
            bIb <- as.numeric(coef %*% info %*% coef)
            lrt <- bIb - (a$r2 * bIb)
